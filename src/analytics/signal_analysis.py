...
def calculate_harmonics_with_phase(
    signal: np.ndarray,
    sampling_freq: float,
    ...
    """
    Calculate harmonics with phase information using FFT.
    ...
    Args:
        ...
        use_window: Apply Hann window to reduce spectral leakage (recommended for non-synchronized sampling)

    Returns:
        Dictionary {harmonic_number: (amplitude, phase_radians)}
    """
    # Apply window function to reduce spectral leakage
    # This compensates for non-integer cycles in the analysis window
    if use_window:
...
def calculate_thd(harmonics: dict[int, float]) -> float:
    ...
    Args:
        harmonics: Dictionary {harmonic_number: amplitude} from get_harmonics
...
def calculate_cpc_components(
    ...
    v_harmonics = calculate_harmonics_with_phase(v_t, sampling_freq, fundamental_freq)
    i_harmonics = calculate_harmonics_with_phase(i_t, sampling_freq, fundamental_freq)
